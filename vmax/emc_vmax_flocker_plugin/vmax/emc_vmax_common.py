...
from cinder.utils import _, _LE, _LI, _LW
from . import emc_vmax_fast
from . import emc_vmax_https
from . import emc_vmax_masking
from . import emc_vmax_provision
from . import emc_vmax_provision_v3
from . import emc_vmax_utils
import cinder.utils as volume_utils
...
# From emc config XML file - V2 only
FASTPOLICY = 'fastpolicy'

# From emc config XML file - V3 only
SLO = 'slo'
WORKLOAD = 'workload'
...
# # Internal-only extra specs, specific to a storage type
COMPOSITETYPE = 'internal_only:compositetype'
MEMBERCOUNT = 'internal_only:membercount'
...
class EMCVMAXCommon(object):
    ...
    def __init__(self, prtcl, version, configuration=None):
        ...
        self.versioninfo = emc_vmax_utils.EMCVMAXVersionInfo(self.utils)
    ...
    def create_volume(self, volume):
        ...
        """
        volumeSize = int(self.utils.convert_mb_to_bits(volume['size']))
        volumeName = volume['id']
        extraSpecs = self._initial_setup(volume)
        self.conn = self._get_ecom_connection()

        # VolumeName naming convention is 'OS-UUID'.
        volumeName = self.utils.get_volume_element_name(volumeName)
        ...
        if 'consistencygroup_id' in volume and volume['consistencygroup_id']:
            cgName = self.utils.truncate_string(
                volume['consistencygroup_id'], TRUNCATE_8)
    ...
    def _attach_volume(self, volume, connector, extraSpecs,
        ...
        LOG.info(_LI("Adding volume %(vol)s to masking view %(mvName)s"),
                 {'vol': volumeName,
                  'mvName': maskingViewDict['maskingViewName']})
        if isLiveMigration:
        ...
        if 'hostlunid' not in deviceInfoDict:
            ...
            try:
                if (rollbackDict and
                    ((rollbackDict['fastPolicyName'] is not None) or
    ...
    def extend_volume(self, volume, newSize):
        ...
        if int(originalVolumeSize) > int(newSize):
            exceptionMessage = (_(
                "Your original size: %(originalVolumeSize)s MB is greater "
                "than: %(newSize)s MB. Only Extend is supported. Exiting...")
        ...
        additionalVolumeSize = self.utils.convert_mb_to_bits(
        ...
        newSizeBits = self.utils.convert_mb_to_bits(newSize)
    ...
    def _migrate_rollback(self, volumeInstance,
                          storageSystemName, sourceFastPolicyName,
    ...
    def _is_valid_for_storage_assisted_migration(
        ...
        # Get the pool from the source array and check that it is different
        # from the pool in the target array.
    ...
    def _find_storage_sync_sv_sv(self, snapshot, volume, extraSpecs,
        ...
        if foundsyncname is None:
            ...
            if waitforsync:
                rc = self.utils.wait_for_sync(
                    self.conn, foundsyncname, extraSpecs)
                maxJobRetries = self.utils._get_max_job_retries(extraSpecs)
                if rc == maxJobRetries:
                    exceptionMessage = (_("Copy session for synchronization "
                                          "%(sync)s timed out: rc=%(rc)d")
                                        % {'sync': foundsyncname,
                                           'rc': rc})
                    LOG.error(exceptionMessage)
                    raise exception.VolumeBackendAPIException(
    ...
    def _set_ecom_credentials(self, arrayInfo):
        ...
        if self.ecomUseSSL:
            self.url = ("https://%(ip_port)s"
                        % {'ip_port': ip_port})
            if self.ecomNoVerification is not None:
                if self.ecomNoVerification is True:
                    self.ecomNoVerification = bool("True")
                else:
                    self.ecomNoVerification = bool("")
            else:
                self.ecomNoVerification = bool("")
            ...
            self.url = ("http://%(ip_port)s"
                        % {'ip_port': ip_port})
        self.conn = self._get_ecom_connection()
        self.utils.register_client(self.conn, self.version)
...
#         self.versioninfo.gather_version_info(self.conn)
        return extraSpecs
    ...
    def _create_and_get_unbound_volume(
            self, storageConfigService, compositeVolumeInstanceName,
        ...
        """
        assocPoolInstanceName = self.utils.get_assoc_pool_from_volume(
            self.conn, compositeVolumeInstanceName)
        appendVolumeInstance = self._create_and_get_volume_instance(
            storageConfigService, assocPoolInstanceName, 'appendVolume',
            additionalSize, extraSpecs)
        isVolumeBound = self.utils.is_volume_bound_to_pool(
            self.conn, appendVolumeInstance)
    ...
    def _create_and_get_volume_instance(
            self, storageConfigService, poolInstanceName,
    ...
    def _modify_and_get_composite_volume_instance(
            self, elementCompositionServiceInstanceName, volumeInstance,
    ...
    def _get_or_create_default_storage_group(
            self, storageSystemName, volumeDict, volumeName,
    ...
    def _create_cloned_volume(
        ...
        :raises: VolumeBackendAPIException
        """
        cloneVolume['host'] = sourceVolume['host']
    ...
    def _add_clone_to_default_storage_group(
        ...
        defaultStorageGroupInstanceName = (
            self._get_or_create_default_storage_group(
                storageSystemName, cloneDict, cloneName,
                fastPolicyName, extraSpecs))
        if defaultStorageGroupInstanceName is None:
            exceptionMessage = (_(
    ...
    def _delete_volume(self, volume):
        ...
        if extraSpecs[ISV3]:
            ...
            rc = self._delete_from_pool(storageConfigService, volumeInstance,
                                        volumeName, deviceId,
                                        extraSpecs[FASTPOLICY], extraSpecs)
        return (rc, volumeName)

    def _remove_device_from_storage_group(
            self, controllerConfigurationService, volumeInstanceName,
            volumeName, extraSpecs):
        """Check if a volume is part of any storage group prior to delete.

        Log a warning if the volume is part of any storage group.
    ...
    def _delete_snapshot(self, snapshot):
        ...
        if not extraSpecs[ISV3]:
            ...
            if syncName is None:
                ...
                LOG.debug("Deleting snap relationship:  "
                          "Target: %(snapshot)s.  "
    ...
    def create_consistencygroup(self, context, group):
        ...
        cgName = self.utils.truncate_string(group['id'], TRUNCATE_8)
    ...
    def create_cgsnapshot(self, context, cgsnapshot, db):
        ...
        try:
            ...
            for memberInstanceName in memberInstanceNames:
                ...
                volume = {'size': int(self.utils.convert_bits_to_mbs(
                    volumeSizeInbits))}
    ...
    def delete_cgsnapshot(self, context, cgsnapshot, db):
        ...
        try:
            targetCgName = self.utils.truncate_string(
                cgsnapshot['id'], TRUNCATE_8)
    ...
    def _create_v3_volume(self, volume, volumeName, volumeSize, extraSpecs):
        """Create a volume (V3).
    ...
    def _extend_composite_volume(self, volumeInstance, volumeName,
        ...
        LOG.debug("Extend Volume: %(volume)s  New size: %(newSize)s MBs.",
                  {'volume': volumeName,
                   'newSize': newSize})
        ...
        unboundVolumeInstance = self._create_and_get_unbound_volume(
            storageConfigService, volumeInstance.path,
        ...
        rc, modifiedVolumeDict = (
            self._modify_and_get_composite_volume_instance(
                elementCompositionService, volumeInstance,
    ...
    def _slo_workload_migration(self, volumeInstance, volume, host,
                                volumeName, volumeStatus, newType, extraSpecs):
    ...
    def _migrate_volume_v3(
        ...
        if foundStorageGroupInstanceName is None:
            ...
            self.provision.remove_device_from_storage_group(
                ...
                foundStorageGroupInstanceName, volumeInstance.path,
                volumeName, extraSpecs)
        ...
        targetSgInstanceName = self._get_or_create_storage_group_v3(
            poolName, targetSlo, targetWorkload, storageSystemName, extraSpecs)
    ...
    def _set_v2_extra_specs(self, extraSpecs, poolRecord):
        ...
        try:
            ...
            if int(stripedMetaCount) == 1:
                extraSpecs[COMPOSITETYPE] = CONCATENATED
            elif 1 < int(stripedMetaCount) <= 255:
                extraSpecs[COMPOSITETYPE] = STRIPED
            else:
                exception_message = (_(
                    "Number of members in a metadevice should be "
                    "in the range of [1, 255]. stripecount in "
                    "extra specs is set to %(stripedMetaCount)s.")
        ...
        LOG.debug(
            "There are: %(memberCount)s %(compositeType)s metas in "
            "the extra specs.",
    ...
    def _set_common_extraSpecs(self, extraSpecs, poolRecord):
        ...
        :returns: dict -- the extra specifications dictionary
        """
        if poolRecord['PoolName'] is None:
            self._essential_info_missing('Pool')
        extraSpecs[POOL] = poolRecord['PoolName']
        if poolRecord['SerialNumber'] is None:
            self._essential_info_missing('Array')
        extraSpecs[ARRAY] = poolRecord['SerialNumber']
        if poolRecord['PortGroup'] is None:
            self._essential_info_missing('PortGroup')
    ...
    def _essential_info_missing(self, missingInfo):
        """Throw exception if essential info is missing

        :param missingInfo: the info that is missing
        """
        exception_message = (_(
            "The %(missingInfo)s cannot be null. It must be "
            "configured in the EMC configuration file corresponding "
            "to the Volume Type.") % missingInfo)
        LOG.error(exception_message)
        raise exception.VolumeBackendAPIException(
            data=exception_message)

    def _get_job_extra_specs(self, extraSpecs):
        """Get user defined extra specs around job intervals and retries.

        :param extraSpecs: the extraSpecs
        ...
        try:
            ...
            LOG.debug("Interval is not in the extra specs.")

        try:
            retries = extraSpecs[RETRIES]
            LOG.debug("The user defined retries is : %(retries)s.",
                      {'retries': retries})
        except KeyError:
            LOG.debug("Retries is not in the extra specs.")
    ...
    def _create_v2_replica_and_delete_clone_relationship(
        ...
        sourceName = sourceVolume['name']
        cloneName = cloneVolume['name']
        cloneName = self.utils.get_volume_element_name(cloneName)
    ...
    def _create_replica_v3(
        ...
        volume = {'size':
                  int(self.utils.convert_bits_to_mbs(volumeSizeInbits))}
        _rc, volumeDict, _storageSystemName = (
            self._create_v3_volume(
        ...
        try:
            ...
            if syncInstanceName is not None:
                ...
                _rc, job = self.provisionv3.break_replication_relationship(
                    self.conn, repServiceInstanceName, syncInstanceName,
    ...
    def _find_ip_protocol_endpoints(self, conn, storageSystemName):
        ...
        for ipProtocolEndpointInstance in ipProtocolEndpointInstances:
            if storageSystemName in (
                    ipProtocolEndpointInstance.path['SystemName']):
                propertiesList = ipProtocolEndpointInstance.properties.items()
                for properties in propertiesList:
                    if properties[0] == 'IPv4Address':
                        cimProperties = properties[1]
                        foundIpAddress = cimProperties.value
                        if foundIpAddress == '127.0.0.1' or (
                                foundIpAddress == '0.0.0.0'):
                            # Don' add it to the list
                            LOG.debug("Omitting %(foundIpAddress)s from list",
                                      {'foundIpAddress': foundIpAddress})
                        else:
                            foundIpAddresses.append(foundIpAddress)
    ...
    def manage_existing(self, volume, external_ref):
        ...
        if extraSpecs[FASTPOLICY]:
            ...
            raise exception.VolumeBackendAPIException(
                data=exceptionMessage)
        # Check if the volume is attach by checking if in any masking view.
        volumeInstanceName = (
            self.utils.find_volume_by_device_id_on_array(self.conn,
                                                         arrayName, deviceId))
        sgInstanceName = (
            self.utils.get_storage_group_from_volume(
                self.conn, volumeInstanceName, 'To-fix'))
        ...
        if repSessionInstanceName is not None:
            exceptionMessage = (_(
                "Unable to import volume %(deviceId)s to cinder. "
        ...
        LOG.debug("Storage pool existing volume is in: %(volPool)s, "
                  "Storage pool current cinder manages: %(cinderPool)s.",
    ...
    def manage_existing_get_size(self, volume, external_ref):
        ...
        :param volume: the volume object including the volume_type_id
        :param external_ref: reference to the existing volume
        :returns: size of the volume in MB
        """
        LOG.debug("Entering common manage_existing_get_size."
                  "Volume: %(volume)s.",
                  {'volume': volume})
        arrayName, deviceId = self.utils.get_array_and_device_id(volume,
                                                                 external_ref)
        ...
        mbSize = int(byteSize) / units.Mi
        LOG.debug(
            "Size of volume %(deviceID)s is %(volumeSize)s MB.",
            {'deviceID': deviceId,
             'volumeSize': mbSize})
        return mbSize
    ...
    def update_consistencygroup(self, group, add_volumes,
        ...
        cg_name = self.utils.truncate_string(group['id'], TRUNCATE_8)
    ...
    def create_consistencygroup_from_src(self, context, group, volumes,
        ...
        :param sorted_snapshots: snapshots of the source volumes
        """
        ...
        try:
            ...
            for volume, snapshot in zip(volumes, snapshots):
                volumeSizeInbits = int(self.utils.convert_mb_to_bits(
            ...
            sourceCgName = self.utils.truncate_string(
                cgsnapshot['id'], TRUNCATE_8)
