...
class EMCVMAXMasking(object):
    ...
    def get_or_create_masking_view_and_map_lun(self, conn, maskingViewDict,
        ...
        try:
            ...
            exceptionMessage = (_(
                ...
                "The exception was %(ex)s. Attempting rollback.")
                % {'maskingViewName': maskingViewDict['maskingViewName'],
                   'ex': e})
    ...
    def _get_and_remove_from_storage_group_v2(
        ...
        if defaultStorageGroupInstanceName is None:
            exceptionMessage = (_(
                "Volume not in the default storage group for FAST policy: "
                "%(fastPolicyName)s.")
                % {'fastPolicyName': fastPolicyName})
            LOG.error(exceptionMessage)
    ...
    def _create_storage_group(
        ...
        if isV3:
            ...
            if (fastPolicyName and defaultStorageGroupInstanceName and
                    foundStorageGroupInstanceName):
                ...
                if assocTierPolicyInstanceName is None:
                    ...
                    LOG.info(_LI("Associating Storage group: "
                                 "%(storageGroupName)s with "
                                 "fast policy: %(fastPolicyName)s."),
                             {'storageGroupName': storageGroupName,
                              'fastPolicyName': fastPolicyName})
    ...
    def _get_storage_group_from_masking_view(
        ...
        if foundView is not None:
            ...
            LOG.debug("Masking view: %(view)s.  "
                      "DeviceMaskingGroup: %(masking)s.",
    ...
    def remove_and_reset_members(
            ...
            volumeName, extraSpecs, connector=None):
    ...
    def _last_vol_in_SG(
            self, conn, controllerConfigService, storageGroupInstanceName,
            storageGroupName, volumeInstance, volumeName, extraSpecs):
        """Necessary steps if the volume is the last in the SG.

        1. Check is volume in a masking view.
        2. If it is delete the MV, remove the volume from the SG, and
           delete the SG.
        3. If it is not in a MV, remove the volume from the SG and
           delete the SG.
