...
APPLICATION_ID = "Openstack_Cinder_VMax"
VENDOR_ID = "EMC Corp"
...
class EMCVMAXUtils(object):
    ...
    def find_storage_configuration_service(self, conn, storageSystemName):
        """Get the storage configuration service.
    ...
    def wait_for_sync(self, conn, syncName, extraSpecs=None):
        ...
        LOG.debug("wait_for_sync returned %(rc)d.", {'rc': rc})
        return rc
    ...
    def convert_mb_to_bits(self, strMbSize):
        """Convert MB(string) to bytes(string).

        :param strMbSize: string -- The size in MB
        :returns: string -- The size in bytes
        """
        strBitsSize = six.text_type(int(strMbSize) * 1024 * 1024)

        LOG.debug("Converted %(strMbSize)s GBs to %(strBitsSize)s Bits.",
                  {'strMbSize': strMbSize, 'strBitsSize': strBitsSize})
    ...
    def convert_bits_to_mbs(self, strBitSize):
        """Convert bytes(string) to GB(string).

        :param strBitSize: string -- The size in bytes
        :returns: int -- The size in MB
        """
        mbSize = int(strBitSize) / 1024 / 1024
        return mbSize
    ...
    def compare_size(self, size1Str, size2Str):
        ...
        :returns: int -- size1MBs - size2MBs
        """
        size1MBs = self.convert_bits_to_mbs(size1Str)
        size2MBs = self.convert_bits_to_mbs(size2Str)

        return size1MBs - size2MBs
    ...
    def process_exception_args(self, arg, instanceName):
        ...
        if code == pywbem.CIM_ERR_NOT_FOUND:
    ...
    def is_clone_licensed(self, conn, capabilityInstanceName):
        ...
        :param capabilityInstanceName: the replication service capabilities
            instance name
        :returns: boolean -- True if licensed and enabled; False otherwise.
        """
    ...
    def _clean_node_value(self, nodeValue):
        """Removes any leading or trailing spaces, tabs, and newlines.

        :param nodeValue: the node value
        :returns: the clean nodeValue
        """
    ...
    def _process_tag(self, element, tagName):
        ...
        try:
            ...
            nodeValue = self._clean_node_value(nodeValue)
        except IndexError:
            pass
        return nodeValue
    ...
    def _get_random_portgroup(self, element):
        ...
        if portGroupElements and len(portGroupElements) > 0:
            ...
            return self._get_random_pg_from_list(portGroupNames)

        exception_message = (_("No Port Group elements found in config file."))
        LOG.error(exception_message)
        raise exception.VolumeBackendAPIException(data=exception_message)

    def _get_random_pg_from_list(self, portGroupNames):
        ...
        if portGroupName:
            ...
            exception_message = (_("Random Port Group name not found from "
                                   "list: %(list)s")
                                 % {'list': portGroupNames})
            LOG.error(exception_message)
            raise exception.VolumeBackendAPIException(data=exception_message)
    ...
    def get_associated_replication_from_source_volume(
        ...
        """Find associated replication storage synchronized instance names.
    ...
    def update_provider_location_for_volume(self, volumeInstanceName):
        """Update the provider location property for the volume object
        ...
        keys = {}
        volume = {}
        keys['CreationClassName'] = volumeInstanceName['CreationClassName']
        keys['SystemName'] = volumeInstanceName['SystemName']
        keys['DeviceID'] = volumeInstanceName['DeviceID']
        keys['SystemCreationClassName'] = (
            volumeInstanceName['SystemCreationClassName'])
        provider_location = {}
        provider_location['classname'] = (
            volumeInstanceName['CreationClassName'])
    ...
    def register_client(self, conn, version):
        """Register Cinder VMax driver with SMI

        :param conn: ECOM connection
        :param version: current cinder vmax driver version
        """
        rc = 0
        smiVersion = self.get_smi_version(conn)
        if smiVersion < 800 or self.reg is True:
            return rc
        (versionMajor, versionMinor, versionEdit) = version.split(".")
        versionMajor = self.get_num(versionMajor, '32')
        versionMinor = self.get_num(versionMinor, '32')
        versionEdit = self.get_num(versionEdit, '32')

        regServices = conn.EnumerateInstanceNames(
        ...
        if regServices:
            for regService in regServices:
                ...
                LOG.debug("Register client: regService=%(regService)s"
                          "%(major)d.%(minor)d.%(edit)d, "
                          "rc=%(rc)d, ret=%(ret)s",
                          {'regService': regService,
                           'major': versionMajor, 'minor': versionMinor,
                           'edit': versionEdit, 'rc': rc, 'ret': ret})
            self.reg = True
        return rc
    ...
    def get_all_arrays(self, conn):
        """Get all arrays connected to the ECOM.
        ...
        for storageSystem in storageSystems:
            arrays.append(storageSystem['Name'])
        return arrays


class EMCVMAXVersionInfo(object):
    """Relevant version information

    SMI-S, Pywbem, Microcode
    OS/distribution, release, and version.
    """

    PYTHON_CINDER = 'python-cinder'
    PYTHON_PYWBEM = 'python-pywbem'

    def __init__(self, utils):

        self.version_dict = {'version': 'unknown version',
                             'release': 'unknown release',
                             'vendor': 'unknown vendor',
                             'platform': 'unknown plaform',
                             'smis_version': 'unknown SMI-S version',
                             'pywbem_version': 'unknown Pywbem version',
                             'vmax_microcode_versions': []}
        self.utils = utils
    ...
    def _update_info_from_rpm(self, package):
        """Update package info from RPM.

        RDO, RHEL-OSP, Mirantis on Redhat, SUSE.

        :param package: the python package e.g python_pywbem
        :returns: dict -- package_info
        """
        package_info = {}
        try:
            out, __ = putils.execute("rpm", "-q", "--queryformat",
            ...
            if not out:
                ...
                return False
            parts = out.split()
            package_info['version'] = parts[0]
            package_info['release'] = parts[1]
            package_info['vendor'] = ' '.join(parts[2::])
        except Exception as e:
            LOG.info(_LI('Could not run rpm command: %(msg)s.'), {'msg': e})
        return package_info

    def _update_info_from_dpkg(self, package):
        """Update package info from DPKG.
        ...
        try:
            _vendor = None
            out, __ = putils.execute("dpkg-query", "-W", "-f='${Version}'",
                                     package)
            if not out:
            ...
            if ':' in deb_version:
                __, upstream_version = deb_version.split(':')
                _release = upstream_version
                if '-' in deb_version:
                    deb_revision = deb_version.split('-')[1]
                    _vendor = deb_revision
                    package_info['release'] = _release
                    if _vendor:
                        package_info['vendor'] = _vendor
        except Exception as e:
    ...
    def _update_info_from_version_info(self):
        ...
        try:
            ...
            if ver:
                self.version_dict['version'] = ver
        except Exception:
            pass
        try:
            rel = self._get_version_info_release()
            if rel:
                self.version_dict['release'] = rel
        except Exception:
            pass

    def _update_python_package(self, package):
        """Update python package.

        :param package: the python package e.g python_pywbem
        """
        found_package = self._update_info_from_rpm(package)
        if not found_package:
            self._update_info_from_dpkg(package)
    ...
    def _update_openstack_info(self):
        ...
        self._update_python_package(self.PYTHON_CINDER)

    def _update_vmax_info(self, conn):
        """Update VMAX info.

        :param conn: the connection to the ecom server
        """
        vmax_version_dict = {}
        storageSystemNames = self.utils.get_all_arrays(conn)
        for storageSystemName in storageSystemNames:
            vmax_version_dict['name'] = storageSystemName
            vmax_version_dict['version'] = (
                self.utils.get_firmware_version(conn, storageSystemName))
            self.version_dict['vmax_microcode_versions'].append(
                vmax_version_dict)
        pywbem_version = self._update_python_package(self.PYTHON_PYWBEM)
        if pywbem_version:
            self.version_dict['pywbem_version'] = pywbem_version
        smi_version = self.utils.get_smi_version(conn)
        if smi_version:
            self.version_dict['smis_version'] = smi_version

    def gather_version_info(self, conn):
        """Get all arrays connected to the ECOM.

        :param conn: the connection to the ecom server
        """
        self._update_openstack_info()
        self._update_vmax_info(conn)
