...
class EMCMidRangeBlockDriverLogHandler(logging.Handler):
...
def create_driver_instance(cluster_id, **config):
    ...
    root_logger.addHandler(EMCMidRangeBlockDriverLogHandler())

    log_level = config.get('DEBUG_LEVEL', logging.INFO)
    root_logger.setLevel(log_level)

    if 'backend_type' in config:
        return BACKEND[config['backend_type']](cluster_id, **config)


class EMCBlockAPI(object):
    """Block device driver for EMC MidRange Storage.

    Implements the ``IBlockDeviceAPI`` for interacting with EMC MidRange
    Storage.
    """

    VERSION = '0.1'

    def __init__(self, cluster_id, client, pools, multipath, proto, host_ip):
        """Initialize new instance of the driver.

        :param cluster_id: The cluster ID we are running on.
        :param client:  VNXClient or UnityClient instance to communicate
                        with EMC MidRange array.
        :param pool:    Storage pool to create volume
        :param multipath: True to enable multipath by default
        :param proto:   iSCSI or FC
        :param host_ip: IP of Flocker agent Node.
                        It is used for iSCSI initiator auto-registration.
                        Otherwise the user has to register initiator
                        manually.
        """
        LOG.debug('Entering EMCBlockAPI.__init__')
        ...
        if self.proto == 'FC':
            self.connector = connector.LinuxFibreChannelConnector(
                client, multipath=multipath)
            self.allowed_ports = client.get_fc_targets()
        elif self.proto == 'iSCSI':
            self.connector = connector.LinuxiSCSIConnector(
                client, multipath=multipath)
            self.allowed_ports = client.get_iscsi_targets()
        else:
    ...
    def destroy_volume(self, blockdevice_id):
        LOG.debug('Entering EMCBlockAPI.destroy_volume: blockdevice_id=%s',
                  blockdevice_id)
        ...
        LOG.debug('Exiting EMCBlockAPI.destroy_volume: volume_name=%s',
                  volume_name)
...
class EMCVNXBlockAPI(EMCBlockAPI):
    def __init__(self, cluster_id, **kwargs):
        ...
        storage_pools = kwargs.get('storage_pools')
        multipath = kwargs.get('multipath', True)
        proto = kwargs.get('proto', 'iSCSI')
        host_ip = kwargs.get('host_ip', None)

        self.scheduler = scheduler.CapacityPoolScheduler(_client)

        super(EMCVNXBlockAPI, self).__init__(
            cluster_id, _client, storage_pools, multipath, proto, host_ip)
    ...
    def create_volume_with_profile(self, dataset_id, size, profile_name):
        ...
        # For VNX, only CapacityPoolScheduler is used to get a right pool
        pools = self._get_managed_storage_pools(self.pool_names)
        filter_properties = dict(size=size)
...
@interface.implementer(blockdevice.IBlockDeviceAPI)
@interface.implementer(blockdevice.IProfiledBlockDeviceAPI)
class EMCUnityBlockAPI(EMCBlockAPI):
    def __init__(self, cluster_id, **kwargs):
        LOG.debug('Entering EMCUnityBlockAPI.__init__')
        _client = client.UnityClient(
            ip=kwargs.get('ip'),
            username=kwargs.get('user'),
            password=kwargs.get('password'))
        ...
        super(EMCUnityBlockAPI, self).__init__(
            cluster_id, _client, storage_pools, multipath, proto, host_ip)
        ...
        self.scheduler['profile'] = scheduler.ProfileBasePoolScheduler(_client)
        self.scheduler['capacity'] = scheduler.CapacityPoolScheduler(_client)
    ...
    def create_volume_with_profile(self, dataset_id, size, profile_name):
        ...
        if profile_name:
            ...
            pool = self.scheduler['profile'].filter_one(pools,
                                                        filter_properties)
        else:
            filter_properties = {
                'size': size,
            }
            pool = self.scheduler['capacity'].filter_one(pools,
                                                         filter_properties)
    ...
    def _assure_host(self, host_info):
        ...
        # [iSCSIConnector]
        #   auto-register initiator if host_ip is set in agent.yml.
        #   Otherwise user has to create initiator in Unity manually
        # [FibreChannel]
        #   User has to pre-configure the zone manually

        # Well, host is ready. How about the initiator?
        # What if there is one existed host without initiator?
        self._register_initiator(host, host_info, force_create)
        return host
    ...
    def _register_initiator(self, host, host_info, force_create):
        for initiator in host_info.initiators:
            host.add_initiator(initiator, force_create=force_create)
...
BACKEND = {'vnx': EMCVNXBlockAPI, 'unity': EMCUnityBlockAPI}
